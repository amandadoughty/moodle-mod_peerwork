...
use mod_peerwork\event\assessable_submitted;
use mod_peerwork\event\peer_grade_created;
use mod_peerwork\event\peer_grade_overridden;
use mod_peerwork\event\submission_cleared;
use mod_peerwork\event\submission_created;
use mod_peerwork\event\submission_files_deleted;
use mod_peerwork\event\submission_files_uploaded;
use mod_peerwork\event\submission_updated;
use mod_peerwork\peerwork_plugin;
...
function peerwork_save($peerwork, $submission, $group, $course, $cm, $context, $data, $draftitemid, $membersgradeable) {
    ...
    $event = assessable_submitted::create(['context' => $context]);
    $event->trigger();

    // Create or update the submission.
    [$submission, $draftfiles] = mod_peerwork_save_submission($peerwork, $submission, $group, $context, $data, $draftitemid);
    ...
    if (!empty($lockedpeerids)) {
        [$notlockedpeersql, $notlockedpeerparams] = $DB->get_in_or_equal($lockedpeerids, SQL_PARAMS_NAMED, 'param', false);
    }
    ...
    foreach ($criteria as $criterion) {
        foreach ($membersgradeable as $member) {
            ...
            $event = peer_grade_created::create($params);
        ...
        foreach ($members as $member) {
            ...
            if (array_key_exists($member->id, $peerworkpeers)) {
                ...
                if ($peerworkpeer->grade != $grade || $peerworkpeer->comments != $comment) {
                    ...
                    $event = peer_grade_overridden::create($params);
...
function mod_peerwork_save_submission($peerwork, $submission, $group, $context, $data, $draftitemid) {
    ...
    if (!$skipfile) {
        ...
        if ($deletedhashes) {
            ...
            $event = submission_files_deleted::create($params);
            $event->trigger();
        }
        ...
        if ($addedhashes) {
            ...
            $event = submission_files_uploaded::create($params);
            $event->trigger();
        }
...
function mod_peerwork_clear_submissions($peerwork, $context, $groupid = 0) {
    ...
    foreach ($submissions as $submission) {
        ...
        $event = submission_cleared::create($params);
        $event->add_record_snapshot('peerwork_submission', $submission);
        $event->trigger();
    }
}
 ...
 */
function update_plugin_instance(peerwork_plugin $plugin, stdClass $formdata) {
    if ($plugin->is_visible()) {
